...
export default function Draft({draftDetails, send}: DraftProps) {
    ...
    const { data: watchedPicks } = useQuery({
    ...
    useEffect(() => {
        if (!draftDetails.id || !playersData || !managerPicks || !budgetedPicks || !watchedPicks) return;
        draftSend({
            type: 'draft_loaded',
            ...
            watchedPlayers: watchedPicks,
        });
    }
    , [playersData, draftDetails.id, managerPicks, budgetedPicks, watchedPicks, draftSend]);
